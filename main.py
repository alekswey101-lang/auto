...
# --- ТЕХНИЧЕСКИЙ АВТОСБОР ПО CALLBACK DATA (АНТИ-ПЕТЛЯ) ---
async def twink_collect_logic(client, acc_id):
    ...
    for tick in range(150):
        try:
            # Получаем последние 2 сообщения из истории, чтобы знать, что было шагом ранее
            history = []
            async for m in client.get_chat_history(bot_chat, limit=2):
                history.append(m)
            ...
            msg = history[0] # Текущее открытое сообщение
            ...
            # Проверяем, забиты ли слоты (ищем в текущем или в предыдущем сообщении)
            slots_full = False
            for h_msg in history:
                if h_msg and h_msg.text and "занято слотов: 10/10" in h_msg.text.lower():
                    slots_full = True
                    break

            # ЕСЛИ МЫ ЗНАЕМ, ЧТО УЖЕ НАБРАНО 10/10 ПРЕДМЕТОВ
            if slots_full:
                # Если бот скинул нас в подменю или окно состояния (где есть кнопка назад) — принудительно выходим!
                back_button_found = False
                for row in msg.reply_markup.inline_keyboard:
                    for btn in row:
                        if "вернуться назад" in btn.text.lower() or "назад" in btn.text.lower():
                            print(f"⚖️ [Твинк {acc_id}] Слоты забиты (10/10), но бот открыл подменю. Нажимаю 'Вернуться назад' для выхода...", flush=True)
                ...
                # Если мы в Главном Меню трейда — жмем Готов
                if has_button(msg, "готов"):
                    print(f"⚡ [Твинк {acc_id}] Трейд успешно заполнен (10/10). Нажимаю 'Готов'!", flush=True)
                    await click(client, msg, "готов")
                    return # Полный выход из функции автосбора!

                if "готовность: ✅" in text:
                    print(f"✨ [Твинк {acc_id}] Кнопка 'Готов' успешно зафиксирована. Завершаю поток.", flush=True)
                    return
                
                continue

            # --- ОБЫЧНЫЙ СБОР ПРЕДМЕТОВ (ЕСЛИ ЕЩЕ НЕ 10/10) ---
            buttons = []
            for row in msg.reply_markup.inline_keyboard:
                for btn in row:
                    if btn.callback_data:
            ...
            for btn in buttons:
                ...
                # Игнорируем навигацию во время сбора
            ...
            if single_buttons:
                ...
                if target.callback_data != last_clicked_callback:
                    ...
                    print(f"📦 [Твинк {acc_id}] Шаг Финал: Нажимаю 'Добавить 1 шт.'", flush=True)
                    await client.request_callback_answer(msg.chat.id, msg.id, target.callback_data, timeout=1)
                    await asyncio.sleep(0.5) 
                continue

            # Шаг Б: Главное меню трейда
            if add_buttons:
                target = add_buttons[0]
                if target.callback_data != last_clicked_callback:
                    last_clicked_callback = target.callback_data
                    print(f"➕ [Твинк {acc_id}] Шаг Menu: Нажимаю 'Добавить телефон'", flush=True)
                    await client.request_callback_answer(msg.chat.id, msg.id, target.callback_data, timeout=1)
                    await asyncio.sleep(0.3)
                continue

            # Шаг В: Окно состояния
            if cond_buttons:
                ...
                if target.callback_data != last_clicked_callback:
                    last_clicked_callback = target.callback_data
                    print(f"📱 [Твинк {acc_id}] Шаг Состояние: Выбираю 'Рабочий телефон'", flush=True)
                    await client.request_callback_answer(msg.chat.id, msg.id, target.callback_data, timeout=1)
                    await asyncio.sleep(0.4) 
                continue

            # Шаг Г: Список моделей/редкостей
            if item_buttons:
                ...
                if target.callback_data != last_clicked_callback:
                    last_clicked_callback = target.callback_data
                    print(f"💎 [Твинк {acc_id}] Шаг Выбор: Кликаю по категории/модели [{target.text}]", flush=True)
                    await client.request_callback_answer(msg.chat.id, msg.id, target.callback_data, timeout=1)
                    await asyncio.sleep(0.4) 
                continue

        except Exception as e:
            print(f"⚠️ Ошибка в техническом автосборе твинка {acc_id}: {e}", flush=True)
...
async def process_bot_logic(client, message, acc_id):
    ...
    if acc_id != 2:
        if "предложение обмена" in text or "пришло предложение" in text:
            ...
            if await click(client, message, "trade_accept") or await click(client, message, "принять"):
                print(f"✅ [Твинк {acc_id}] Трейд принят. Включаю технический сборщик...", flush=True)
                asyncio.create_task(twink_collect_logic(client, acc_id))
            return

        # Страховочный чек клика "Готов" для твинка из главного обработчика
        if "10/10" in text and has_button(message, "готов"):
            print(f"⚡ [Твинк {acc_id}] Страховка: Нажимаю 'Готов' в чате.", flush=True)
    ...
    if "❌" in text or "✅" in text:
        if "✅" in text and has_button(message, "готов"):
            print(f"⚡ [ОСНОВА - Акк 2] Твинк готов! Прожимаю 'Готов' на основе...", flush=True)
            await click(client, message, "готов")
            return
...
async def start_bot():
    global clients
    print("🛠 Запуск фермы. Исправлена проверка сквозной истории подменю.", flush=True)
    ...
    print("🚀 Скрипт полностью готов и запущен! Теперь выйдет из любого подменю.", flush=True)
