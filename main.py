...
async def receiver_trade_logic(client, acc_id):
    ...
    for loop_index in range(1, 30):
        ...
        res, current_fp = await execute_menu_step(
            client,
            acc_id,
            "Добавить телефон",
            ["добавить телефон", "trade_add_phone"],
            False,
            current_fp
        )
        ...
        res, current_fp = await execute_menu_step(
            ...
            acc_id,
            "Количество 1шт",
            ["добавить 1 шт.", "trade_add_single"],
            False,
            current_fp
        )
                ...
                async for fresh_msg in client.get_chat_history(bot_chat, limit=1):
                    if fresh_msg.reply_markup:
                        if await click(client, fresh_msg, "принять"):
                            ...
                            asyncio.create_task(
                                receiver_trade_logic(client, acc_id)
                            )
...
async def handle_my_messages(client, message):
    ...
    if cmd == ".ping":
        try:
            await message.edit(
                "🚀 Юзербот запущен! Защита от рассинхрона и Iris Farm активны."
            )
        except:
            pass
    ...
    if cmd in [".trade", ".t", ".т"]:
        ...
        asyncio.create_task(
            sender_confirm_logic(client, acc_id)
        )
    ...
    while True:
        try:
            ...
            if acc_id in [1, 2]:
                ...
                if iris_timer >= 240:
                    print(
                        f"🌙 [Акк {acc_id}] Прошло 4 часа! Отправляю 'фарма'...",
                        flush=True
                    )
                    ...
                    try:
                        ...
                        print(
                            f"⚠️ [Акк {acc_id}] Ошибка Iris Moon: {e}",
                            flush=True
                        )
...
async def start_bot():
    ...
    for i, session in enumerate(SESSIONS):
        ...
        c.add_handler(
            handlers.MessageHandler(handle_my_messages)
        )
        ...
        try:
            ...
            print(
                f"✅ Аккаунт {i+1} запущен: @{me.username}",
                flush=True
            )
