            ...
            if message.reply_markup:
                for row in message.reply_markup.rows:
                    for button in row.buttons:
                        # Проверка по data (техническому ID)
                        current_data = ""
...
async def run_daily_farm(client, acc_id):
    ...
    while True:
        ...
        print(f"⏳ Акк {acc_id}: Ожидание фермы {int(wait_secs/3600)}ч {int((wait_secs%3600)/60)}м", flush=True)
        ...
        try:
            ...
            if await click_inline_button(client, "@phonegetcardsbot", "Снять деньги"):
                print(f"✅ Акк {acc_id}: Деньги сняты через farm_claim", flush=True)
        except Exception as e:
